class Sensor {
  ...
  #getReading(ray, roadBorders, traffic) {
    ...
    // return nearest touch (smallest offset)
    if (touches.length === 0) {
      return null;
    } else {
  ...
  draw(context) {
    for (let i = 0; i < this.rays.length; i++) {
      ...
      // if hit, cut the ray at the intersection, else full ray
      const end = reading || rayEnd;
